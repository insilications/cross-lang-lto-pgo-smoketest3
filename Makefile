...
COMMON_FLAGS_USE=$(RUST_PGO_USE) -Clink-arg=-Wl,--plugin-opt=--debug-pass=Arguments -Cno-prepopulate-passes -Cpasses=name-anon-globals -Clinker-plugin-lto=on -Clink-arg=-flegacy-pass-manager -Zrelro-level=full -Clink-arg=-flto=full -Cprefer-dynamic=no -Clink-arg=-Wl,--mllvm=-inlinehint-threshold=1400 -Clink-arg=-Wl,--mllvm=-inlinedefault-threshold=1400 -Clink-arg=-Wl,--mllvm=-inlinecold-threshold=1400 -Clink-arg=-Wl,--mllvm=-inline-cost-full -Clink-arg=-Wl,--mllvm=-inline-cold-callsite-threshold=1400 -Clink-arg=-Wl,--mllvm=-inline-threshold=1400 -Clink-arg=-Wl,--plugin-opt=-inlinehint-threshold=1400 -Clink-arg=-Wl,--plugin-opt=-inlinedefault-threshold=1400 -Clink-arg=-Wl,--plugin-opt=-inlinecold-threshold=1400 -Clink-arg=-Wl,--plugin-opt=-inline-cold-callsite-threshold=1400 -Clink-arg=-Wl,--plugin-opt=-inline-threshold=1400 -Cllvm-args=-inline-threshold=1400 -Cinline-threshold=1400 -L. --verbose -Clink-arg=--verbose -Clink-arg=-L. -Clink-arg= -Zunstable-options -Clinker=clang -Clink-arg=-fuse-ld=lld -Copt-level=3 -Ccodegen-units=4 -Cpanic=abort -Clto=fat -Ctarget-feature=+crt-static -Znew-llvm-pass-manager=no -Crelocation-model=static -Csave-temps=no -Clink-arg=-Wl,--plugin-opt=save-temps -Clink-arg=-Wl,--save-temps -Clink-arg=--save-temps -Cstrip=debuginfo -Cdebuginfo=2 -Clink-arg=-fsave-optimization-record=yaml -Clink-arg=-foptimization-record-passes=.* -Clink-arg=-Rpass=.* -Clink-arg=-Rpass-missed=.* -Clink-arg=-Rpass-analysis=.* -Clink-arg=-fdiagnostics-show-hotness -Clink-arg=-Wl,--plugin-opt=opt-remarks-filename=myopt.yaml -Clink-arg=-Wl,--plugin-opt=opt-remarks-passes=.* -Clink-arg=-fsave-optimization-record=yaml -Clink-arg=-foptimization-record-passes=.* -Clink-arg=-Rpass=.* -Clink-arg=-Rpass-missed=.* -Clink-arg=-Rpass-analysis=.* -Clink-arg=-fdiagnostics-show-hotness -Clink-arg=-Wl,--plugin-opt=opt-remarks-filename=myopt.yaml -Clink-arg=-Wl,--plugin-opt=opt-remarks-passes=.* -Clink-arg=-Wl,-O3,--sort-common,--as-needed,--lto-partitions=1,--lto-O3 -Ctarget-cpu=native -Cprefer-dynamic=no -Zplt=off -Ztune-cpu=native -Clink-arg=-Wl,-plugin-opt=O3,-plugin-opt=mcpu=native,-plugin-opt=lto-partitions=1 -Clink-arg=-Wl,--lto-O3 -Clink-arg=-Wl,-O2 -Clink-arg=-O3 -Clink-arg=-mno-vzeroupper -Clink-arg=-march=native -Clink-arg=-mtune=native -Crelocation-model=static -Cforce-frame-pointers=off -Cforce-unwind-tables=off -Cpanic=abort -Clink-arg=-Wl,-plugin-opt=-import-instr-limit=400 -Clink-arg=-Wl,--lto-legacy-pass-manager -Clink-arg=-fwhole-program-vtables -Clink-arg=-Wl,--lto-whole-program-visibility  -Zfunction-sections=no -Zprint-llvm-passes=no -Clink-args='-mllvm -enable-new-pm=0' -Zcombine-cgu=yes -Clink-arg=-Wl,--mllvm=-inline-deferral -Cembed-bitcode=yes -Clinker-plugin-lto
